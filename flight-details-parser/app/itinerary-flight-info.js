...
function ItineraryFlightInfo(options) {
  ...
  for(let i = 0; i < flightCount; i++) {
    ...
    // logger.debug(`ItineraryFlightInfo: optionMap for flight ${i} (number ${options.flight_num[i]} dump: ${JSON.stringify(optionMap[i])}`);
    /*
    let tc = optionMap[i].travel_class;
    ...
    */
    item.travel_class = options.flightInfo_travelClass[i];
    this.details.push(item);
  }
}

function toMap(options) {
  // logger.debug(`toMap: options dump: ${JSON.stringify(options)}`);
  const optionMap = [];
  const flightCount = options.flight_num.length;
  for(let i = 0; i < flightCount; i++) {
    const item = {};
    Object.keys(options).forEach(key => {
      ...
      // logger.debug(`toMap: key ${key}'s item ${item[key]}; options: <${options[key]}>`);
    });
    optionMap.push(item);
  }
  return optionMap;
